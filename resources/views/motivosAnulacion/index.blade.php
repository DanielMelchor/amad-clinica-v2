...
@endsection
@section('title', 'Motivos de Anulación')
...
@section('content')
										...
										<tbody>
											@foreach($pMotivosAnulacion as $pMA)
												<tr class="text-center" style="font-size: 12px;">
													...
													@endif
													<td>
														@php $Id= Crypt::encrypt($pMA->id); @endphp
														<a href="#" class="btn btn-xs btn-warning rounded-circle elevation-4" title="Editar Registro" onclick="fn_edicion('{{ $Id }}')"><i class="fas fa-edit"></i></a>
													</td>
												</tr>
											@endforeach
										</tbody>
	...
	<div class="modal fade" id="agregarModalCenter" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  		<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    		<div class="modal-content">
      			<form role="form" id="formaNuevoRegistro" method="POST" action="{{route('motivoanulacion_grabar')}}">
	      			@csrf
	      			<div class="card">
	...
	<div class="modal fade" id="editarModalCenter" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="editarModalCenterTitle" aria-hidden="true">
  		<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    		<div class="modal-content">
      			<form role="form" method="POST" action="{{route('motivoanulacion_actualizar')}}">
	      			@csrf
	      			<div class="card">
	        			<div class="card-header" style="background-color: #F4F6F7;">
...
@section('js')
    ...
    <script type="text/javascript">
		...
		function fn_edicion(id){
			console.log(id);
			$.ajax({
	        	url: "{{ route('motivoanulacion_editar') }}",
		        type: "POST",
		        dataType: 'json',
