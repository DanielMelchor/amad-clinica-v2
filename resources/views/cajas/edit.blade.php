                ...
                <div class="col-12 col-lg-10">
                    <div class="card shadow-sm">
                        ...
                        <div class="card-header d-flex align-items-center" style="background-color: #E1E8ED;">
                            <h6 class="mb-0 flex-grow-1 font-weight-bold text-truncate">Edición de Caja</h6>
                        ...
                        <div class="card-body p-2 p-md-3">
                            ...
                            <input type="hidden" id="caja_id" name="caja_id" value="{{ old('caja_id', $Id) }}">
                            <div class="row justify-content-center">
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input type="text" class="form-control" 
                                                   ...
                                                   required value="{{ old('caja_nombre', $pCaja->nombre_maquina ?? '') }}">
                                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-12 col-sm-6 mb-2">
                                            <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                                                <input type="checkbox" class="custom-control-input" id="editar_documento" name="editar_documento" value="S" @if($pCaja->editar_documento == '1') checked @endif>
                                                <label class="custom-control-label" for="editar_documento">Editar Documento</label>
                                            </div>
                                        </div>
                                        <div class="col-12 col-sm-6 mb-2">
                                            <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                                                <input type="checkbox" class="custom-control-input" id="estado" name="estado" value="1" @if($pCaja->estado == '1') checked @endif>
...
@section('js')
    ...
    <script type="text/javascript">
        var nLineaT = 0;
        const tipoDocumentos = @json('tipo_documentos');

        //===================================================================
        // Agregar Resolución
        //===================================================================
        function fnAgregarResolucion(datos = null) {
            ...
            const nuevaFila = `
                <tr>
                    <input type="hidden" name="resoluciones[${nLineaT}][id]" value="${r.id || ''}">
                    <td>
                        <select class="form-control form-control-sm select2-dinamico" 
                                name="resoluciones[${nLineaT}][tipo_documento_id]" required>
                            <option value="">Seleccionar...</option>
                            @foreach($tipo_documentos as $td)
                                <option value="{{ $td->id }}" ${r.tipo_documento_id == {{ $td->id }} ? 'selected' : ''}>
                                    {{ $td->descripcion }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <td>
                        <input type="text" class="form-control form-control-sm text-uppercase" 
                               name="resoluciones[${nLineaT}][serie]" value="${r.serie}">
                    </td>
                    <td>
                        <input type="number" class="form-control form-control-sm numero" 
                               name="resoluciones[${nLineaT}][inicial]" value="${r.correlativo_inicial}">
                    </td>
                    <td>
                        <input type="number" class="form-control form-control-sm numero" 
                               name="resoluciones[${nLineaT}][final]" value="${r.correlativo_final}">
                    </td>
                    <td>
                        <input type="number" class="form-control form-control-sm numero" 
                               name="resoluciones[${nLineaT}][ultimo]" value="${r.ultimo_correlativo}">
                    </td>
                    <td class="text-center">
                        <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                            <input type="checkbox" class="custom-control-input" 
                                   id="est_${nLineaT}" 
                                   name="resoluciones[${nLineaT}][estado]" value="1" ${r.estado == 1 ? 'checked' : ''}>
                            <label class="custom-control-label" for="est_${nLineaT}"></label>
                        </div>
                    </td>
                    <td class="text-right">
                        <button type="button" class="btn btn-xs btn-outline-danger rounded-circle elevation-2 btn-eliminar-fila">
            ...
            $fila.find('.select2-dinamico').select2({
                theme: 'bootstrap4',
                width: '100%'
            });
        ...
        $(document).ready(function() {
            const resoluciones = @json($resoluciones); // Los datos que vienen del controlador

            if (resoluciones && resoluciones.length > 0) {
                resoluciones.forEach(function(item) {
                    // Llamamos a la función pasando el objeto de la iteración
                    fnAgregarResolucion(item);
                });
            }
        });
