...
@endsection
@section('content')
    <form class="form-horizontal" role='form' method="POST" id="cajaForm" name="cajaForm" action="{{ route('actualizar_caja')}}">
        @csrf
        <div class="card">
                ...
                <div class="row">
                    <div class="input-group mb-1 col-lg-4 col-sm-4 offset-lg-1 offset-sm-1">
                        <div class="icheck-primary">
                            <input type="checkbox" id="editar_documento" name="editar_documento" @if($pCaja->editar_documento == 1) then checked @endif /> 
                ...
                <div class="row">
                    <div class="form-group offset-md-1">
                        <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                            &nbsp;<input type="checkbox" class="custom-control-input" id="estado" name="estado" value="A" @if($pCaja->estado == 1) then checked @endif>
            ...
            $.ajax({
                ...
                success: function(response){
                    response.forEach(function(item, index) {
                        //console.log(item);
                        fnAgregarResolucion();
                        let tipoInput    = $('#resoluciones\\['+index+'\\]\\[tipo_documento_id\\]');
                        let serieInput   = $('#resoluciones\\['+index+'\\]\\[serie\\]');
                        let inicialInput = $('#resoluciones\\['+index+'\\]\\[inicial\\]');
                        let finalInput   = $('#resoluciones\\['+index+'\\]\\[final\\]');
                        let ultimoInput  = $('#resoluciones\\['+index+'\\]\\[ultimo\\]');
                        let estadoInput  = $('#resoluciones\\['+index+'\\]\\[estado\\]');
                        if (tipoInput.length > 0) {
                            tipoInput.val(item['tipo_documento_id']).trigger('change');
                        }
                        if (serieInput.length > 0) {
                            serieInput.val(item['serie']);
                        }
                        if (inicialInput.length > 0) {
                            inicialInput.val(item['correlativo_inicial']);
                        }
                        if (finalInput.length > 0) {
                            finalInput.val(item['correlativo_final']);
                        }
                        if (ultimoInput.length > 0) {
                            ultimoInput.val(item['ultimo_correlativo']);
                        }
                        if (item['estado'] == 1) {
                            estadoInput.prop('checked', true);
                        }else{
                            estadoInput.prop('checked', false);
                        }
                        /*if (tipoSelect.length > 0) {
                            tipoSelect.val(item['unidad_medida_id']).trigger('change'); // Usamos trigger para aplicar select2
                            console.log(tipoSelect.val(item['unidad_medida_id']).value);
                        }else{
                    ...
                    /*for (var i = 0; i < response.length; i++) {
                        fnAgregarResolucion();
                        let tipoSelect = $('#resoluciones\\['+i+'\\]\\[tipo_documento_id\\]');
                        if (tipoSelect.length > 0) {
                            tipoSelect.val(item['unidad_medida_id']).trigger('change'); // Usamos trigger para aplicar select2
                        }
                        //document.getElementById("resoluciones["+i+"][tipo_documento_id]").value = response[i]['tipo_documento_id'];
                        document.getElementById("resoluciones["+i+"][serie]").value = response[i]['serie'];
                        document.getElementById("resoluciones["+i+"][inicial]").value = response[i]['correlativo_inicial'];
                        document.getElementById("resoluciones["+i+"][final]").value = response[i]['correlativo_final'];
                        document.getElementById("resoluciones["+i+"][ultimo]").value = response[i]['ultimo_correlativo'];
                        if (response[i]['estado'] == '1') {
                            document.getElementById("resoluciones["+i+"][estado]").checked = true;
                        }
                        //console.log(response[i]['serie']);
                    }*/
                },
